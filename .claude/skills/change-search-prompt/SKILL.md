...
| D1 track config (`/api/config`) | one track's stored prose - role line, how the resume is framed, every fit rule that screens a posting out (fit clause, fit step) | that one track, immediately, no deploy |
| The person's location settings (`/api/config`) | where they'd work, as they typed it - `search_locations`, `priority_locations`, `excluded_locations`, `location_note`. Step 5 prints them; how they combine is fixed in `prompt.js` | every search of that person, on the next run, no deploy |
