/**
 * The write indicator. Every write path must set it: with optimistic updates the
 * row has already changed on screen, so this is the only thing telling "saved"
 * from "about to be rolled back".
 *
 * A module store, like prefs: every mutation writes it and one element reads it.
 */
