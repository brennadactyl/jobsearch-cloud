...
- A `.docx` that can't be read (not really a zip, password-protected, damaged) or that yields fewer than 50 words (a scanned image, a template) is a `422 { error, words? }` naming why, and nothing is stored - neither file.
