...
- `GET /api/data` -> `{ user, updated, leads[], applications[], screened[], tracks[], settings }` - everything the page renders, for this person only. `user` is `{id, name}`. **Screened rows are a window**: the last 90 days, with `screened_window: { days, older }` saying how many older rows are kept but not sent - a search screens dozens of postings a night, and the older ones are read by nobody, since what they are for (stopping the next run re-finding a posting) happens server-side on every write. Nothing is deleted. **`?screened=all`** serves every row, with `days: 0`: the window hides rows the person still owns, and this is how they can be asked for - without it, reading what fell outside means an operator and a backup.
