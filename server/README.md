...
- `POST /api/screened/kinds` - **`ADMIN_TOKEN` as the Bearer, not a session** - body `{ user, rows: [{ id, kind }], dryRun? }` -> `{ dryRun, set, skipped, unknown, rows: [{ id, was, now, outcome }] }` - fills in the kind on rows screened before the column existed. **It refuses a kind outside the list**, naming the value and the row, unlike `POST /api/screened` which coerces: this creates no rows, so a typo is worth catching rather than filing under `other`. Answers per row - `unknown` for an id that is not this account's screened row, `already set` for one classified already, `set`/`would set` for one filled in - because a total hides which rows those were. Writes only where the kind is still empty, so a re-run after a partial write cannot overwrite a corrected value. `dryRun` reports the same rows and writes nothing. Retired once the backfill is done.
