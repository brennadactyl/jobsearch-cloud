...
- `POST /api/areas/fill` - **`ADMIN_TOKEN` as the Bearer, not a session** - body `{ user, leads?: [{ id, area }], applications?: [{ id, area }], dryRun? }` -> `{ dryRun, leads: { set, skipped }, applications: { set, skipped }, area_cleared }` - the one-time fill that gives the named account's existing leads and applications their `area` (`docs/location-settings-plan.md`). The areas are worked out beforehand with the page's own matcher; each is stored only if it names one of the account's ranked entries (the same check as `POST /api/leads`, counted in `area_cleared` otherwise), and only on a row whose area is still empty - one a run has filed since is kept and counted in `skipped`. Rows of another account match nothing. One transaction; `dryRun` counts and writes nothing.
