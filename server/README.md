...
leads, applications, page titles and locations.
...
- `GET /api/data` -> `{ user, updated, leads[], applications[], screened[], tracks[], settings }` - everything the page renders, for this person only. `user` is `{id, name}`. **Two rules about screened rows, answering different questions.** *What is sent* answers "was this ever a posting of theirs?": a row recording something the person once had is sent whatever its kind - one carrying a `found` date, a `delisted` one, one added by hand - because the page counts a week's found postings as its leads plus the screened rows that were leads and went away. Withheld is a run's rejection of a posting nobody ever had whose kind isn't settings-caused: `dead`, `duplicate`, unclassified. *What is counted* answers "did their own settings reject it?": `screened_counts: { <search>: <count> }`, over the whole table rather than the window and over the narrower set in `src/validate.js` `SCREENED_BY_RULES`, with a search that has no such row **absent rather than 0** - nothing counted is not the same as a month that turned nothing away. Rows are never deleted either way. A run record carries `screened_by_rules` beside `screened_added`: a number for every run recorded since the column existed, `0` included, and **`null` for a night recorded before it** - that night turned postings away and nobody recorded which were rules, which is not the same fact as a night whose rules turned nothing away. On top of the first rule, the last 90 days, with `screened_window: { days, older }` where `older` follows that same rule - a search screens dozens of postings a night, and the older ones are read by nobody, since what they are for (stopping the next run re-finding a posting) happens server-side on every write. Nothing is deleted. **`?screened=all`** serves every row, with `days: 0`: the window hides rows the person still owns, and this is how they can be asked for - without it, reading what fell outside means an operator and a backup.
- `POST /api/leads` - body `{ "on": "YYYY-MM-DD", "leads": [ {search, company, title, location, url, fit, team, setup, comp} ] }` -> `{ added, duplicates, excluded, area_cleared }`. Appends only postings this person doesn't already have under that search. Dedup matches the canonical URL (`src/url.js`) as well as `UNIQUE(user_id, search, url)`, so a `?gh_jid=` suffix, a slug or a tracking param is still a duplicate. Dedup is per search, not per tab: a branched run can't file one posting into two of its tabs, while two independent tracks each keep their own row. Companies on `excluded_companies` are dropped. `on` is the caller's local date and stamps `found`/`verified`. Never touches an existing row's status or notes. `team`/`setup`/`comp` are optional - omit them unless the posting states them. Other Details fields (referral, resume, lastContact, nextAction*, link) are accepted but are for the person to enter; search scripts don't send them. Every distinct `search` must be one of this person's configured tracks (a fed tab counts); an unknown key is a 404 naming it, and nothing in the payload is inserted. **Each lead may carry `area`**, the ranked place it falls in: it is stored only when it names an entry of the person's `priority_locations` (split on commas, trimmed, compared ignoring case), spelled as that entry is, and empty otherwise - never refused, since losing a lead over its area is the worse failure. `area_cleared` counts the areas emptied. An application made from the lead carries its area.
- `POST /api/screened` - body `{ "search": "SWE", "on": "YYYY-MM-DD", "screened": [ {search, url, company, title, location, reason, kind} ] }` -> `{ added, duplicates, excluded, kinds_coerced? }`. **`kind`** is which sort of rejection it was, from the closed list in `src/validate.js` (`SCREENED_KINDS`: delisted, dead, duplicate, out-of-scope, pay-below-floor, wrong-level, wrong-role, contract, other) - the part a page groups and counts by, while `reason` stays the sentence about that one posting. A kind that isn't on the list is **stored as `other` and named back** in `kinds_coerced` (`{ <sent value>: <times> }`, absent when nothing was coerced) rather than refusing the row: the row is what stops the next run re-finding the posting, so no grouping word is worth losing one. An item sent without a kind stores `""`, which means nobody has said. **A run never sends `delisted`**: a posting that went away was a lead on this person's board first, so `POST /api/delist` stamps that kind itself, and a lead someone clears off their board through `POST /api/delete-leads` carries no kind at all - that is a decision, not one of the sorts of rejection a search makes. Records postings a search looked at and rejected (dead on arrival, out of scope, wrong level, duplicate), so the next run's dedup skips them. Same canonical dedup, exclusion handling and configured-track check as `/api/leads` (unknown key: 404, nothing inserted); the check applies to the `search` sent. **Each row is filed under the tab it was judged for**: one search fills several tabs, and a row filed under the root reads as though that tab's search rejected it. A row`s `search` is kept when it names a track in the running search's own group - the body's `search`, or the row's own for a caller that sends none - and falls back to that group's root otherwise, counted in `tabs_filed_at_root` (`{ <sent key>: <times> }`), since a misfiled row is worth more than a lost one. For a search that fills more than one tab the reply also carries **`tabs_named`** and **`tabs_of`** - how many rows said which tab they were judged for, of how many sent - so a run that stops naming tabs reads as `tabs_named: 0` rather than being indistinguishable from a night where every row belonged to the root. A single-tab search is not asked: naming nothing is the whole truth there. **Send `on`**: `/api/runs` counts a day's screened rows by it, and without it an evening run's rows take the worker's UTC date and the run records screening nothing. `reason` is free text except `"posting taken down"`, which is reserved for delistings and rewritten if a caller sends it.
...
- `POST /api/applications/autofill` - body `{ filled: [{id, company, title, location, area, team, setup, comp, note?}], failed: [{id, reason}] }` -> `{ filled, failed, unmatched: [id], area_cleared }`. `area` is checked as on a lead (see `POST /api/leads`), and `area_cleared` counts those emptied. A partial read is a `filled` entry with a `note` saying what's missing; `failed` is for a posting that yielded nothing. **Writes only into columns that are still empty**, so anything the person typed wins. Only rows still at `''` change, so an id in `unmatched` was deleted or already reported - nothing to retry. A `filled` entry with no usable field still marks the row read. `filled` and `failed` are final; the note or failure reason is stored in `autofill_note`, which the page shows on that row.
...
- `POST /api/screened/kinds` - **`ADMIN_TOKEN` as the Bearer, not a session** - body `{ user, rows: [{ id, kind }], dryRun? }` -> `{ dryRun, set, skipped, unknown, rows: [{ id, was, now, outcome }] }` - fills in the kind on rows screened before the column existed. **It refuses a kind outside the list**, naming the value and the row, unlike `POST /api/screened` which coerces: this creates no rows, so a typo is worth catching rather than filing under `other`. Answers per row - `unknown` for an id that is not this account's screened row, `already set` for one classified already, `set`/`would set` for one filled in - because a total hides which rows those were. Writes only where the kind is still empty, so a re-run after a partial write cannot overwrite a corrected value. `dryRun` reports the same rows and writes nothing. **A write that does not name every unclassified row is refused** (`409`, saying how many it missed and some of their ids) unless the caller sends `leaveRest: true`: a tool reading `GET /api/data` instead of `?screened=all` sees only what the page is served, so it would classify a fraction of the table and report success. Declining to judge a row is legitimate and `leaveRest` says so; not knowing a row exists is not. The refusal is decided before anything is written. Retired once the backfill is done.
...
- `POST /api/settings` - body `{ resumes?: { <search>: <path> }, search_locations?, excluded_locations?, priority_locations?, location_note?, display_title?, pronouns?, excluded_companies?, searches?: { <key>: { label?, role_search_line?, fit_clause?, fit_disqualifier?, paused?, pay_floor?, pay_floor_unit? } } }` -> `{ resumes: { <search>: { documents, profile_pending } }, locations: { <key>: <stored value> }, settings: { display_title, pronouns, excluded_companies }, searches: { <key>: { label, role_search_line, fit_clause, fit_disqualifier, paused_since, paused, pay_floor, pay_floor_unit } } }`. The account panel's write, and the mirror of `POST /api/writeup`: any other key is `400 { error, field }` naming it. It is the panel's **one Save**, so a request may carry a resume choice, a place edit, a page title and a tab rename together; everything it writes takes effect on the next read, and `settings` and `searches` in the reply are read back after the write rather than echoed. **What the panel names**: `display_title` and each `label` are text, trimmed, refused empty, and capped at 120 and 60 characters; `pronouns` is one the prompt knows (`she/her`, `he/him`, `they/them`) or empty; `excluded_companies` is a list of at most 500 names, and an empty list means "exclude no one". **What the panel says a search is for**: `searches` writes a search's tab name and what it looks for - `role_search_line`, `fit_clause` and `fit_disqualifier`, each optional, each stored as typed and read by the prompt on that search's next run, nothing composed and nothing regenerated. `fit_clause` and `fit_disqualifier` accept `""` and clear; `label` and `role_search_line` are refused empty, since an empty `role_search_line` is how a search says it has never been written up. The roles line is capped at 300 characters and each rule at 2,000. Any other field of a track is `400 { error, search, field }` naming it - the rest of a track's config, the tab order and the track list itself are `POST /api/config`'s, so a panel save can never drop a search - and a key this account doesn't have is `404 { error, search, field: "label" }`. The reply's `searches` carries every search, so one reply refreshes the panel. **The lowest pay worth showing**: `pay_floor` is the amount as typed (at most 40 characters, `""` clears it) and `pay_floor_unit` is `year` or `hour` beside it. Nothing parses a number out of the amount - `src/prompt.js` states both in step 7 and the run judges a posting against them - so "180k", "$180,000" and "£140,000" are all kept as written, the last saying its own currency. The two are set together and cleared together: a write leaving one without the other is `400 { error, search, field: "pay_floor" }`, judged on what the save leaves, so sending one while the other is stored is an ordinary edit. `POST /api/config` takes the same pair for an operator. **Pausing**: `paused` is `true` or `false` and the server stamps `paused_since` or clears it, so the page never sends a time and pausing an already-paused search keeps the instant it carries. Only the track that runs a search can be paused - a tab follows its root - so `paused` on a fed tab is a `400` naming the search to pause instead. Each search in the reply carries its own `paused_since` and the resolved `paused` a reader shows. **Where the search looks**: each location list and the note is stored as typed, trimmed at the ends, and nothing splits or checks the places in it - the prompt reads each list as written (`docs/location-settings-plan.md`). A list over 4,000 characters, a note over 1,000, or a value that isn't text is `400` naming the key, and nothing in the request is written. A key left out keeps its value; `""` clears it. A save that would leave both `search_locations` and `priority_locations` without an entry is `400` naming `search_locations`; an empty `search_locations` alone means only the ranked places. Points each named search at a stored resume under `resumes/`; a Word file (or its text's path) is stored as its text, which is what a run reads. The resume replaces every `resumes/` entry in that search's `documents` list, keeping anything else - a reference file - in place. A change marks the search's profile stale (`profile_stale_since`, `resume_was` in `docs/schema.md`) once the search has been written up - before that it has no profile, and its write-up reads whichever resume is current, so `profile_pending` is false; naming the resume it already reads changes nothing. Every search is checked before any is written, and the writes are one transaction. Refusals are `{ error, search, field: "resume" }`: `404` for a search this person doesn't have or a file that isn't stored, `400` for a tab another search fills (the error names that search) or a path outside `resumes/`, `422` for a file a search can't read.
...
- `GET /api/config` -> `{ tracks: [{key, label, full_description, sort_order, last_run, ...search config}], settings }` - this person's config: the track tabs, labels, display title, the location settings and staleness threshold the client renders from, plus the per-track search config and prose settings the prompt is composed from. Each track's `last_run` is its `search_runs` row (`{at, on, status, leads_added, screened_added, delisted, swept, note}`; all-empty means never recorded).
- `POST /api/config` - body `{ tracks?, display_title?, overview_label?, applications_label?, all_leads_label?, stale_run_hours?, search_locations?, excluded_locations?, priority_locations?, location_note?, excluded_companies?, footer_note?, pronouns? }`. `tracks`, if present, **replaces this person's whole track list** and keeps `search_runs` 1:1 with it; leads under a removed track keep their `search` value and lose their tab, never deleted. It never touches anyone else's tracks. Each track entry may carry its search config: `role_search_line`, `target_companies` (array, or a string when the list has prose structure), `search_note`, `resume_line`, `fit_clause`, `fit_disqualifier`, `fit_filter_step`, `leads_note`, `doc_file`, `doc_summary`, `doc_update_line`, `intro_note`, `report_line`, `screened_examples`, `schedule_time`, `fed_by`, `paused_since`, and `documents` - a list of the document paths the search reads besides its `doc_file` (at most 20, each a valid document path, or a `400`). A list that changes must leave the search something to read - at least one `.txt`, `.md` or `.pdf`, and no Word file, which a run reads through its extracted `.txt` - or it is a `400 { error, field: "documents" }`; `POST /api/writeup` applies the same rule. A list identical to the stored one, and a tab another search fills, are exempt, so `GET /api/config` read and posted back is always accepted. **`paused_since` pauses a search**: an ISO 8601 instant stops it, stored normalised, and `""` resumes it. It is set on the track that runs the search - a tab another search fills is paused with it, so a `paused_since` on a fed track is a `400 { error, field: "paused_since" }` naming the track to pause instead. Pausing changes nothing else: leads, screened rows, sweeps, the doc and the rotation stay as they are, and `GET /api/config` serves each track a resolved `paused` beside it.
...
- `GET /api/prompt/:key` -> **`text/plain`** - the daily search prompt for that track, composed from the config above (see `src/prompt.js`). This is what `run-search.ps1` pipes into the CLI, and the fastest way to check what a search will do. `?doc_budget=<bytes>` sets how much step 8b says a run may add to its doc: `run-search.ps1` passes the budget it enforces, and a value that isn't a whole number from 100 to 20000 falls back to the default (1000) rather than failing. 404s on a key this person doesn't have, and 409s on one with `fed_by` set (the error names the track that fills it), on a paused search (the error says since when, and `paused_since` carries it), or on one not yet written up. Each 409 body carries a `code` - `fed_tab`, `paused` or `not_written_up` - so a runner can tell them apart without matching the sentence.
