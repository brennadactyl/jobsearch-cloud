...
This changes `client/` (the panel) and `server/` (a field-scoped settings
route). No run is involved: the panel edits values only. It builds on
[instant-setup-plan.md](instant-setup-plan.md), whose ownership split decides
what each edit is allowed to touch.
...
Values, written straight through and in force at once:

| Field | |
|---|---|
| Page title | `display_title` |
| Pronouns | `settings.pronouns` |
| Where the search looks | the three location lists and the note, as chips, the same component the setup form uses |
| Companies you'd never work for | `excluded_companies`, as chips |
| What each search is called | each track's `label` |
| Which resume each search uses | `tracks.documents` |

Nothing here waits for a run. The answers the overnight run turns into prose -
what each search looks for, and the pay floor - are not edited here: on a built
account, rewriting them regenerates each track's doc and loses what the nightly
runs have accumulated in it. Editing what a search looks for is its own backlog
item, and it goes through `POST /api/config` rather than through the answers.
...
- **`POST /api/settings`** (session token) accepts only the fields the form
  ...
  configuration.
- **The stored setup answers are never rewritten here.** `POST /api/intake`
  stays one-shot, and no route replaces the answers, so no edit in this panel
  can put a built account back through a build.
- `verify-local` proves: the settings route cannot write a write-up field, and
  a save leaves every track key, lead and application untouched.
...
Adding or removing a search, editing the answers the run turns into prose,
editing what a search looks for, recovering a setup that failed, editing the
prose the run wrote, a self-service password reset for someone locked out, and
deleting your own account. The middle three are on
